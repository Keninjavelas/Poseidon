...
describe('validateEnv — environment variable startup guard', () => {
  let exitSpy;
  let errorSpy;
  ...
  it('calls process.exit(1) when a single required var is missing', () => {
    const env = {
      DATABASE_URL: 'postgresql://localhost/test',
      MQTT_BROKER_URL: 'mqtt://localhost:1883',
      WS_PORT: '8080',
      // CORS_ORIGIN intentionally absent
    };
    validateEnv(env);
    expect(exitSpy).toHaveBeenCalledWith(1);
  });

  it('logs [FATAL] message identifying the missing variable', () => {
    const env = {
      DATABASE_URL: 'postgresql://localhost/test',
      MQTT_BROKER_URL: 'mqtt://localhost:1883',
      // WS_PORT and CORS_ORIGIN absent
    };
    validateEnv(env);
    const loggedMessages = errorSpy.mock.calls.map((c) => c[0]);
    expect(loggedMessages.some((m) => m.includes('[FATAL]'))).toBe(true);
    expect(loggedMessages.some((m) => m.includes('WS_PORT'))).toBe(true);
  });
    ...
    fc.assert(
      fc.property(
        ...
        (missingVars) => {
          ...
          // Build env with all vars present, then delete the missing ones
          const env = {
            DATABASE_URL: 'postgresql://localhost/test',
            MQTT_BROKER_URL: 'mqtt://localhost:1883',
            WS_PORT: '8080',
